...
Read the help page of the `summary()` function. Then, use this function and combine it with logical selection to find out what the mean, minimum and maximum values are of the LipoP score for the four different classes (CYT, SpI, SpII, TMH).
Hint: select each subgroup in the dataframe using logical selection on the rows, and only the "LipoP_Score" of the columns 

```{r}
summary(protein_data[protein_data$LipoP_Localisation == "CYT", 16])
summary(protein_data[protein_data$LipoP_Localisation == "SpI", 16])
summary(protein_data[protein_data$LipoP_Localisation == "SpII", 16])
summary(protein_data[protein_data$LipoP_Localisation == "TMH", 16])
```
...
plot(protein_data$SignalP_Smax, protein_data$LipoP_Score,
     xlab = "SignalP_Smax", ylab = "LipoP_Score")
```
...
hist(protein_data$TMHMM_Length, breaks = 30)
```
